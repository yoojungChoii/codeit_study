...
function findEmail() {
  ...
  // 원래함수.find(function(현재의 요소) { 실행할 문장} )
  // Arrow 함수로 바꾸면 function과 중괄호를 생략 가능하다. 
  const user = data.find((el) => 
  // 두 값을 찾을 때 두 조건 모두를 만족시키려면 &&를 꼭 써야한다. 
    nameValue === el.userName &&  phoneValue === el.phoneNumber);
